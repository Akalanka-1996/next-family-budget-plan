...
export async function POST(req: Request) {
  ...
  try {
    ...
    let invitedUser = await prisma.user.findUnique({ where: { email: parsed.data.email } })
    ...
    const existing = await prisma.familyMember.findUnique({ where: { userId_familyId: { userId: invitedUser.id, familyId: parsed.data.familyId } } })
